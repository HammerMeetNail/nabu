...
func seedService(t *testing.T, logs []chorelog.ChoreLog) (*stats.Service, *stubChoreStore) {
	...
	for _, l := range logs {
		d := l.CompletedAt
		_, err := logSvc.LogChore(ctx, l.HouseholdID, l.UserID, l.ChoreID, l.Note, l.Indicators, l.IndicatorVolumes, &d, l.SlotHour, &d, l.VolumeML, nil)
		if err != nil {
...
func TestGetTopChores_Limit(t *testing.T) {
	...
	for i, ch := range cs.chores {
		...
		for j := 0; j < count; j++ {
			...
			_, err := logSvc.LogChore(ctx, 1, 10, ch.ID, "", nil, nil, &d, nil, &d, nil, nil)
			if err != nil {
				t.Fatalf("seed log: %v", err)
			}
		}
