...
test.describe('Home Grid: Rendering', () => {
  test('home grid renders chore cards after login', async ({ page }) => {
    ...
    // 17 default seeded chores
    await expect(page.locator('.home-chore-card')).toHaveCount(17);
...
// ─── Home Grid: Indicator Log Sheet ──────────────────────────────────────────

test.describe('Home Grid: Indicator Log Sheet', () => {
  /**
   * Finds and clicks the "Change Baby" card which has indicator labels.
   */
  ...
  test('tapping a card with indicator labels opens the log sheet', async ({ page }) => {
    await setupWithChores(page);
...
test.describe('Home Grid: Jiggle Mode', () => {
  ...
  test('in jiggle mode cards have the reorder drag attribute', async ({ page }) => {
    ...
    await expect(reorderCards).toHaveCount(17);
  });
});
