...
- **Rate-limit authentication endpoints.**  `/api/auth` has a strict limiter (`RATE_LIMIT_AUTH_MAX`, default 30/min/IP). A permissive global backstop on all `/api/*` (`RATE_LIMIT_GLOBAL_MAX`, default 600/min/IP) is enabled only when `TRUSTED_PROXY_CIDRS` is set, so it can't 429 the whole user base behind an unconfigured proxy. Behind a proxy/tunnel, set `TRUSTED_PROXY_CIDRS` so client IPs (and audit-log IPs) are real.
