...
```bash
podman ps --format '{{.Names}}' | grep choresy
```
...
make down
COMPOSE_PROJECT_NAME=choresy-A make local

# Second stack: use a unique project name and override ports
COMPOSE_PROJECT_NAME=choresy-B \
...
```bash
COMPOSE_PROJECT_NAME=choresy-A make down
COMPOSE_PROJECT_NAME=choresy-B make down
```
