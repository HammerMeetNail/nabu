...
| **Baby Care** |
| Feed Baby volume | `schedule.js`, `feed-baby-volume.spec.js`, `log-amount-layout.spec.js` | `Views/LogSheet.swift`, `Views/StatsView.swift` | `/api/logs` | Built | PWA amount logs (including medications) group the numeric input and unit selector side by side with aligned labels and 44px controls. Indicator logs, including medications with custom types and the seeded Feed Baby activity, put their one shared unit selector beside the amount column instead of in a separate full-width row. Chromium and WebKit regressions cover narrow widths, 200% zoom, save/reload/edit, and preserving amounts when types are toggled. Native iOS retains Form presentation with the same shared log-level unit behavior; no API or model changes. |
