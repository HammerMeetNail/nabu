...
| **Activity** |
| History list (paginated) | `activity-data.js`, `activity-context.spec.js` | `API/ActivityStore.swift`, `Views/ActivityView.swift`, `ActivityTests.swift` | `/api/logs/history` | Built | One query owner covers entry, search, refresh, edit, deletion and pagination. Old responses cannot restore deleted rows or clear newer loading/errors. Local chore filters retain the loaded page boundary; canceled pagination can retry immediately. PWA Activity amounts use each chore’s custom unit (including per-indicator amounts), matching iOS; Both clients show persistent custom-unit labels beside selected indicator inputs, including while editing. Native search uses the system search field; empty/error retry controls remain visible. |
