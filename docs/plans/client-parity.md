...
| Schedule edit (sparse PATCH) | `schedule.js`, `schedule-tab.spec.js` | `API/ScheduleStore.swift`, `RequestEncodingTests.swift` | `/api/schedules/{id}` | Built | Unchecking the end date sends explicit null; native and browser persistence regressions cover setting and clearing it. |
