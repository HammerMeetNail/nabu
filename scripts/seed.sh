#!/bin/sh
# Seed a test user in the Nabu app
# Creates test@nabu.local / "correct horse battery"
set -e
...
CSRF_TOKEN=$(curl -sf -c - "${APP_BASE_URL}/api/auth/login" 2>/dev/null | grep nabu_csrf | awk '{print $NF}')
